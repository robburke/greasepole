    ...
    public bool ProcessAI()
    {
        if (!SplashscreenLoadingFinished)
        {
            if (!LoadedLongSounds)
            {
                ...
                // Load all sounds (100%) - they're needed for title music and narrator
                if (Globals.SoundService.LoadLongSounds(100))
        ...
        {
            Globals.Debug($"[DEBUG] BackButtonPressed! State: {nGameState}, CurrentGameState: {_CurrentGameState}");
            if (nGameState == (int)GameStates.STATETITLE || 
                (_CurrentGameState == GameStates.STATETITLE && nGameState == (int)GameStates.STATENOCHANGE))
            {
                Globals.Debug("[DEBUG] Exiting Game from Title");
                // Already at title - exit
                bGameTerminate = true;
            }
            else
            {
                Globals.Debug("[DEBUG] Going Back to Title");
                // Not at title - go back to title
                if (nGameState == (int)GameStates.STATENOCHANGE)
                {
                    // Consuming the input is CRITICAL here.
                    // Otherwise, InitTitle() runs, swaps sprites to Title sprites (including Exit button),
                    // and then the rest of THIS frame's logic runs ssIcons.Think(), triggering the Exit button!
                    Globals.InputService.ConsumeBackInput();
                    InitTransition((int)GameStates.STATETITLE); // Use Transition to get the Jacket Slam effect!
                    bProcessingAI = false; // Reset flag since we are aborting frame
                    Globals.Debug("[DEBUG] Aborting Frame after InitTransition");
                    return true; // Abort this frame's logic so we don't accidentally trigger Exit button with same click
                }
            }
        }
        ...
        bool aiRanThisFrame = nAIQueue > 0;
        while (nAIQueue > 0)
        {
            nAIQueue--;
            ...
            // NOTE: Input Update is called once per Godot frame in Main._Process
            // Do NOT call it here - it would clear pending click states between AI iterations
            ...
            // Signal to input service that ONE AI frame completed, so click events can be consumed for this frame
            Globals.InputService.OnAIFrameComplete();
...
//        AIMethods.lSound[((int)ASLList.lsndNARRATOR_OPTIONINIT)].Play(SoundbankInfo.volHOLLAR);
    ...
    public bool Initialize_GameSprites(bool bDemo)
    {
        ...
        // Skin Tones
        byte[] skinToneDefault = new byte[] { 248, 208, 152 }; // Light/Peach
        byte[] skinTonePale = new byte[] { 255, 223, 191 };    // Very Pale
        byte[] skinToneTan = new byte[] { 222, 156, 89 };      // Tan/Olive
        byte[] skinToneMediumBrown = new byte[] { 172, 116, 68 }; // Medium Brown
        byte[] skinToneDarker = new byte[] { 134, 84, 39 };    // Darker Brown
        byte[] skinToneDarkest = new byte[] { 80, 51, 25 };    // Darkest Brown
        byte[] skinToneRosy = new byte[] { 235, 180, 160 };    // Rosy/Reddish

        List<byte[]> skinTones = new List<byte[]> 
        { 
            skinToneDefault, skinTonePale, skinToneTan, 
            skinToneMediumBrown, skinToneDarker, skinToneDarkest, skinToneRosy 
        };

        // Shirt Colors
        byte[] shirtToneDefault = new byte[] { 95, 0, 95 }; // Purple
        byte[] shirtTone2 = new byte[] { 66, 10, 69 };      // Dark Purple
        byte[] shirtTone3 = new byte[] { 136, 31, 142 };    // Magenta
        byte[] shirtTone4 = new byte[] { 109, 51, 112 };    // Light Purple
        
        // New Shirt Colors - All shades of Purple/Violet (Darkened per user request)
        byte[] shirtDeepViolet = new byte[] { 75, 0, 130 };
        byte[] shirtDarkOrchid = new byte[] { 153, 50, 204 };
        byte[] shirtBlueViolet = new byte[] { 138, 43, 226 };
        byte[] shirtPlum = new byte[] { 160, 90, 160 };      // Was 221, 160, 221 (too light)
        byte[] shirtIndigo = new byte[] { 85, 0, 110 };
        byte[] shirtMediumPurple = new byte[] { 130, 80, 190 }; // Was PastelPurple 180, 130, 240

        List<byte[]> shirtColors = new List<byte[]>
        {
            shirtToneDefault, shirtTone2, shirtTone3, shirtTone4,
            shirtDeepViolet, shirtDarkOrchid, shirtBlueViolet, 
            shirtPlum, shirtIndigo, shirtMediumPurple
        };
        ...
        for (i = 0; i < AIMethods.gnNumFroshInPit; i++)
        {
            ...
            // Randomize Skin Tone
            // 20% chance to keep default (no shader cost if we handled it that way, but we map all now)
            // Actually, GodotRenderingService checks equivalence, so mapping Default->Default is 0 cost.
            byte[] chosenSkin = skinTones[R.Next(skinTones.Count)];
            
            // Randomize Shirt Color
            byte[] chosenShirt = shirtColors[R.Next(shirtColors.Count)];

            // Apply Skin Tone
            if (chosenSkin != skinToneDefault) 
            {
                tmpSprite.ReplaceRGB = skinToneDefault; 
                tmpSprite.SubstituteRGB = chosenSkin;
            }
            
            // Apply Shirt Color (independent of skin attributes, but logic in GodotRenderingService handles only one pair per sprite currently?)
            // WAIT. TSprite only has ONE ReplaceRGB/SubstituteRGB pair.
            // We cannot do BOTH skin and shirt replacement on the same sprite with the current TSprite structure.
            // The Original code had "else if" blocks, suggesting it was EITHER skin OR shirt.
            // But realistically, a Frosh has both skin AND a shirt.
            ...
            // So originally, a specific frosh had EITHER a skin tint OR a shirt tint. Not both.
            // This is a limitation of the original engine likely, or just how it was coded.
            // TSprite has: public byte[] ReplaceRGB; public byte[] SubstituteRGB;
            // It only supports ONE replacement.
            
            // To support mixing, we'd need to upgrade TSprite and the Shader to support multiple replacements.
            // OR, we just stick to the "Either/Or" logic but randomize it better so it looks mixed.
            // OR (Advanced): We modify the shader to take arrays, but that's complex for this task.
            
            // Decision: Randomize whether we tint the SKIN or the SHIRT for any given Frosh.
            // This will still result in a diverse looking crowd.
            // 50% chance for Skin tint, 50% chance for Shirt tint.
            // And within those, we pick a random color.
            ...
            {
                // Shirt Tint
                // Note: The shirt replacement works by replacing the DEFAULT shirt color (Purple)
                if (chosenShirt != shirtToneDefault)
                {
                    tmpSprite.ReplaceRGB = shirtToneDefault;
                    tmpSprite.SubstituteRGB = chosenShirt;
                }
            }
